Ext.define('Tualo.tualojs.data.calculation.field.EinheitFaktor', {
    extend: 'Ext.data.field.Number',
    alias: [
        'data.field.tualo_calculation_einheit_faktor'
    ],
    lastQuery: null,
    convert: function (currentValue, record) {
        let me = this;
        let doQuery = true;

        console.log('***EinheitFaktor convert start', currentValue, record);
        if (me.lastQuery != record.get('einheit'))
            if (!Ext.isEmpty(record.get('einheit'))) {
                if (typeof record.modified == 'undefined') return currentValue;
                if (record.modified) {
                    let type = record.get('einheit');
                    let fn = async () => {
                        let request = await fetch('./ds/mengeneinheiten/read/' + type);
                        if (!request.ok) {
                            Ext.toast({
                                html: 'Fehler beim Abrufen ' + type + ' Daten',
                                title: 'Fehler',
                                width: 200,
                                align: 't'
                            });
                            return;
                        }
                        let data = await request.json();
                        if (!data.success) {
                            let msg = data.msg;
                            if (!msg) msg = "Leider ist ein unbekannter Fehler aufgetreten.";
                            Ext.toast({
                                html: msg,
                                title: 'Fehler',
                                width: 200,
                                align: 't'
                            });
                            return;
                        }
                        me.lastQuery = type;


                        record.set('einheit_faktor', data.data[0].faktor * 1.0);
                    };
                    fn();
                }
            }
        return currentValue;
    },
    critical: true,
    persist: true,
    depends: ['einheit'],
});

