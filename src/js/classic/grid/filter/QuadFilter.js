...
Ext.define('Tualo.grid.filters.filter.TriFilter', {
     ...
     * @private
     */
    menuItems: ['lt', 'gt', '-', 'eq', 'neq'],

    constructor: function (config) {
        var me = this,
            stateful = false,
            filter = {},
            filterGt, filterLt, filterEq, filterNeq, value, operator;
        ...
        filterNeq = me.getStoreFilter('neq');

        if (filterLt || filterGt || filterEq || filterNeq) {
        ...
        filter.neq = filterNeq || me.createFilter({
            operator: 'neq',
            value: (!stateful && value && Ext.isDefined(value.neq))
                ? value.neq
                : null
        }, 'neq');
    ...
    setValue: function (value) {
        ...
        if ('eq' in value) {
            ...
            if ('neq' in value) {
                ...
                if (v || v === 0) {
                    add.push(filters.neq);
                    filters.neq.setValue(v);
                }
                else {
                    remove.push(filters.neq);
                }
            }
        }
