Ext.define('Tualo.grid.features.CheckedFixedSummary', {
    /* Begin Definitions */

    extend: 'Ext.grid.feature.AbstractSummary',

    alias: 'feature.tualo_checkedfixedsummary',
    ...
    getSummary: function (store, type, field, group) {
        ...
        if (type) {
            ...
            switch (type) {
                ...
                case 'sum':
                    let v = this.sum(item, field);
                    this.grid.fireEvent('summary', 'sum', field, v);
                    return v;

                case 'average':
                    return this.average(item, field);
    ...
    count: function (store) {
        var c = 0, a,
            ...
            d = b.length;
        for (a = 0; a < d; a++) {
            if (b[a].get(this.checkedFieldName) !== !0) {
                continue
            }
            c += 1
        }
    ...
    sum: function (store, field) {
        ...
        for (a = 0; a < e; a++) {
            ...
            result += b[a].get(field)
        }
        return result
    },
    average: function (a, b) {
        return this.sum(a, b) / this.count(a)
    },
